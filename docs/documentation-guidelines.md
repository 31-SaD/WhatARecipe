...
Every project convention must be documented as a standalone Markdown file inside the `docs/` folder, organized by area (`testing/`, `backend/`, etc.). Each document follows a fixed structure with these sections in order: Convention, Benefits, Examples (good and bad), Real world examples, and Related agreements.
